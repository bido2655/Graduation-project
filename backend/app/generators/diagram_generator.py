...
async def generate_plantuml_code(description: str, diagram_type: str, include_relations: bool, analysis: dict = None) -> str:
    ...
    if diagram_type == "class":
        ...
        if analysis is None:
            analysis = await analyze_for_activity_diagram(description)
        return generate_activity_diagram(analysis)
    elif diagram_type == "bpmn":
        if analysis is None:
            analysis = await analyze_for_bpmn_diagram(description)
        return generate_bpmn_diagram(analysis)
    else:
        if analysis is None:
            analysis = await analyze_for_class_diagram(description)
        return generate_class_diagram(analysis, include_relations)
